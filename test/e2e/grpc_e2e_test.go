...
func TestGRPCPhase1E2E(t *testing.T) {
	...
	_, err = registryClient.Register(context.Background(), &etcdiscv1.RegisterRequest{Input: registrysvc.RegisterInput{Instance: model.Instance{Namespace: "prod-core", Service: "payment-api", InstanceID: "node-1", Address: "127.0.0.1", Port: 8080}}})
	require.NoError(t, err)

	discoveryClient := etcdiscv1.NewDiscoveryServiceClient(conn)
	discoverResp, err := discoveryClient.Discover(context.Background(), &etcdiscv1.DiscoverRequest{Input: discoverysvc.SnapshotInput{Namespace: "prod-core", Service: "payment-api"}})
	require.NoError(t, err)
	require.Len(t, discoverResp.Items, 1)

	configClient := etcdiscv1.NewConfigServiceClient(conn)
	_, err = configClient.PutConfig(context.Background(), &etcdiscv1.ConfigPutRequest{Input: configsvc.PutInput{Item: model.ConfigItem{Scope: model.ConfigScopeService, Namespace: "prod-core", Service: "payment-api", Key: "timeout.request", Value: "1000", ValueType: model.ConfigValueDuration}}})
	require.NoError(t, err)
	...
	_, err = registryClient.Register(context.Background(), &etcdiscv1.RegisterRequest{Input: registrysvc.RegisterInput{Instance: model.Instance{Namespace: "prod-core", Service: "agent-api", AgentID: "agent-1", InstanceID: "inst-1", Address: "10.0.0.9", Port: 9090}}})
	require.NoError(t, err)
	a2aClient := etcdiscv1.NewA2AServiceClient(conn)
	_, err = a2aClient.UpsertAgentCard(context.Background(), &etcdiscv1.UpsertAgentCardRequest{Input: a2asvc.PutInput{Card: model.AgentCard{Namespace: "prod-core", AgentID: "agent-1", Service: "agent-api", Capabilities: []string{"tool.search"}, Protocols: []string{"grpc"}, AuthMode: model.AuthModeStaticToken}}})
	require.NoError(t, err)
	a2aResp, err := a2aClient.DiscoverCapabilities(context.Background(), &etcdiscv1.DiscoverCapabilitiesRequest{Input: a2asvc.DiscoverInput{Namespace: "prod-core", Capability: "tool.search"}})
	require.NoError(t, err)
